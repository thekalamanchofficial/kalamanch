...
export const likeRouter = router({
  ...
  bulkLikePost: protectedProcedure.input(bulkLikeSchema).mutation(async ({ input }) => {
    try {
      if (!input) {
      ...
      for (const { postId, iterationId, liked, postStatus } of input.likes) {
        ...
        if (liked) {
          ...
          if (postId) {
            ...
            iterationLikeCountChanges[iterationId] =
              (iterationLikeCountChanges[iterationId] ?? 0) + 1;
          }
        } else {
  ...
  getUserLikes: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        const likedIterationIds = userLikes
          .map((like) => like.iterationId)
          .filter((iterationId) => iterationId !== null);
  ...
  getUserLikedPost: protectedProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        const formattedLikedPost = likedPost
          .map((like) => like.post)
