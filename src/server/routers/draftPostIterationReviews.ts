...
export const draftPostIterationReviewsRouter = router({
  saveDraftPostIterationReviewers: protectedProcedure
    .input(
      yup.object({
  ...
  getDraftPostIterationsToReview: protectedProcedure
    .input(
      yup.object({
  ...
  getDraftPostIterationsSentForReview: protectedProcedure
    .input(
      yup.object({
        limit: yup.number().min(1).default(5),
        skip: yup.number().min(0).default(0),
    ...
    .query(async ({ input }) => {
      try {
        ...
        const uniqueIterations = Array.from(
          new Map(draftPostIterations.map((item) => [item.iteration.id, item])).values(),
        );
